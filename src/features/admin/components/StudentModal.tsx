...
function validate(form: FormState, isEditing: boolean) {
  ...
  if (!form.codigo_estudiante.trim()) {
    errors.codigo_estudiante = "Ingresa el codigo del estudiante.";
  }
...
export function StudentModal({
  ...
  const title = useMemo(
    () =>
      student
        ? `Editar estudiante ${student.full_name}`
        : "Nuevo estudiante",
    [student],
  );
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  updateField("codigo_estudiante", event.target.value)
                }
                type="text"
